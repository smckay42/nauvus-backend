...
class TestListCreatorAndDispatcherLoad(APITestCase):
    def setUp(self):
        self.load_url = "/api/v1/loads/list-user-loads/"

        self.user = get_user_model().objects.create_user(username="dispatcher_test", first_name="Disaptcher Testing",
    ...
    def test_list_user_load_returns_one_load(self):
        ...
        self.assertEqual(response.json().get('data')[0].get('current_status'), 'booked')
        self.assertEqual(response.json().get('data')[1].get('destination').get('city'), 'Miami')
        self.assertEqual(response.json().get('data')[1].get('current_status'), 'available')
        self.assertEqual(response.status_code, 200)


    def test_retrieve_user_load(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.get(f'{self.load_url}{self.load.id}/')
        self.assertEqual(response.json().get('data').get('destination').get('city'), 'Miami')
