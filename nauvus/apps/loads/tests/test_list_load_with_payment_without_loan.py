...
class TestListLoadWithPaymentWithoutLoad(APITestCase):
    ...
    def test_list_user_load_returns_load_content(self):
        response = self.client.get(self.url)
        self.assertEqual(response.data.get('results')[0].get('payment_to_date'), 600.0)
        self.assertEqual(response.data.get('results')[0].get('remain_payment'), 390.0)
