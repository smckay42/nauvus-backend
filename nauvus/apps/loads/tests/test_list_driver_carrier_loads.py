...
class TestListDriverCarrierLoad(APITestCase):
    def setUp(self):
        ...
        self.carrier_user = CarrierUser.objects.create(user=self.user, carrier=self.carrier, is_current_organization=True)
    ...
    def test_list_user_load_returns_load_content(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.get(self.load_url)
        self.assertEqual(response.json().get('data')[0].get('origin').get('city'), 'New York')
        self.assertEqual(response.status_code, 200)
